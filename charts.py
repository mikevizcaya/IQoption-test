...
def build_event_window(analyzed: pd.DataFrame, source_index: int, mode: str):
    ...
    if mode in ("inicio", "fin"):
        ...
        if mode == "inicio":
            ...
            # Para no usar información futura de la vela seleccionada,
            # anulamos indicadores calculados con CLOSE/HIGH/LOW de esa vela.
            for col in [
                "lsma_low_7", "lsma_close_7", "wma_close_7",
                "di_plus", "di_minus", "adx"
            ]:
                if col in window.columns:
                    window.loc[idx, col] = np.nan
