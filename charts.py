...
def rank_adx_final_smoothers(
    ...
    """
    Prueba automáticamente distintos métodos/períodos para la etapa DX→ADX
    y los ordena por MAE contra valores de IQ Option introducidos por el usuario.

    iq_reference debe incluir:
      - Hora
      - IQ ADX
    """
    dx = _compute_dx_series(analyzed, di_period=7)
    ...
    time_to_idx = {}
    for idx, t in time_series.items():
        time_to_idx[t] = idx
    ...
    # Mantener sólo horas que realmente existen en el dataset.
    ...
    methods = ["DX", "RMA", "RMA_FIRST", "SMA", "EMA"]

    for method in methods:
        periods = [1] if method == "DX" else range(min_period, max_period + 1)

        for period in periods:
            if method == "DX":
                series = dx
                label = "DX sin suavizar"
            else:
                series = _apply_final_smoother(dx, method, period)
                display_method = {
                    "RMA": "RMA",
                    "RMA_FIRST": "RMA primer valor",
                    "SMA": "SMA",
                    "EMA": "EMA",
                }[method]
                label = f"{display_method}{period}"
