...
def calculate_adx(df: pd.DataFrame, period: int = 7, di_period: int = 7) -> pd.DataFrame:
    """
    ADX calibrado para aproximar la curva mostrada por IQ Option.
    ...
    La elección EMA(2) proviene del ajuste contra los valores observados
    en IQ Option en el tramo de validación 19:56–20:00.
    """
    out = df.copy().reset_index(drop=True)
    high = pd.to_numeric(out["high"], errors="coerce").astype(float)
    low = pd.to_numeric(out["low"], errors="coerce").astype(float)
    ...
    denom = (pdi + mdi).replace(0, np.nan)
    dx_ratio = (pdi - mdi).abs() / denom
    # Ajuste final calibrado contra IQ Option: DX suavizado con EMA(2).
    # DI y ATR permanecen en 7 periodos con suavizado Wilder/RMA.
    adx = 100.0 * dx_ratio.ewm(span=2, adjust=False, min_periods=1).mean()
...
def build_adx_components_table(
    ...
    dx_ratio = (pdi - mdi).abs() / denom
    dx = 100.0 * dx_ratio
    # ADX de producción: EMA(2) aplicada a DX.
    adx = 100.0 * dx_ratio.ewm(span=2, adjust=False, min_periods=1).mean()
...
def _apply_final_smoother(dx: pd.Series, method: str, period: int) -> pd.Series:
    """
    Aplica sólo el suavizado FINAL DX→ADX.
    """
    method = method.upper().strip()

    if method == "RMA":
        return _rma(dx / 100.0, period) * 100.0
    if method == "RMA_FIRST":
        return _rma_first(dx / 100.0, period) * 100.0
    if method == "SMA":
        return _sma(dx, period)
    if method == "EMA":
        return _ema_standard(dx, period)
    if method == "DX":
        return dx.copy()
...
def rank_adx_final_smoothers(
    analyzed: pd.DataFrame,
    iq_reference: pd.DataFrame,
    min_period: int = 1,
    max_period: int = 14,
) -> pd.DataFrame:
    """
    Adaptador para la interfaz Streamlit.

    Usa `sweep_adx_final_smoothing`, que mantiene fijo DI=7 y prueba
    únicamente el suavizado final DX→ADX con:
      - DX sin suavizar
      - RMA
      - SMA
      - EMA
    para periodos dentro del rango solicitado.
    """
    refs = iq_reference.copy()
    refs["Hora"] = refs["Hora"].astype(str).str.strip()
    refs["IQ ADX"] = pd.to_numeric(refs["IQ ADX"], errors="coerce")
    refs = refs.dropna(subset=["IQ ADX"])

    if refs.empty:
        return pd.DataFrame()

    real_values = {
        row["Hora"]: float(row["IQ ADX"])
        for _, row in refs.iterrows()
    }

    raw = sweep_adx_final_smoothing(
        df=analyzed,
    ...
    def _label(row):
        if row["metodo"] == "DX sin suavizar":
            return "DX sin suavizar"
        period = int(row["periodo"]) if pd.notna(row["periodo"]) else ""
        return f"{row['metodo']}{period}"

    ranking = pd.DataFrame({
        "Método": raw.apply(_label, axis=1),
        "Puntos": raw["puntos"].astype(int),
        "MAE": raw["MAE"],
        "RMSE": raw["RMSE"],
        "Sesgo": raw["sesgo"],
        "Error máximo": raw["error_max"],
    })

    for col in ["MAE", "RMSE", "Sesgo", "Error máximo"]:
        ranking[col] = pd.to_numeric(ranking[col], errors="coerce").round(4)

    return ranking.reset_index(drop=True)


def build_best_smoothers_chart(
    analyzed: pd.DataFrame,
    iq_reference: pd.DataFrame,
    ranking: pd.DataFrame,
    theme: str = "Blanco",
    ...
    """
    Grafica IQ Option contra los mejores candidatos del ranking.
    """
    dx = _compute_dx_series(analyzed, di_period=7)

    time_series = pd.to_datetime(analyzed["datetime_mexico"]).dt.strftime("%H:%M:%S")
    time_to_idx = {t: idx for idx, t in time_series.items()}

    refs = iq_reference.copy()
    refs["Hora"] = refs["Hora"].astype(str).str.strip()
    refs["IQ ADX"] = pd.to_numeric(refs["IQ ADX"], errors="coerce")
    refs = refs.dropna(subset=["IQ ADX"])
    refs = refs[refs["Hora"].isin(time_to_idx)]

    fig = go.Figure()

    if refs.empty or ranking.empty:
        fig.update_layout(title="Sin datos suficientes para comparar")
        _apply_chart_theme(fig, theme)
        return fig

    ref_times = refs["Hora"].tolist()
    ref_indices = [time_to_idx[t] for t in ref_times]
    x = pd.to_datetime(analyzed.loc[ref_indices, "datetime_mexico"])

    fig.add_trace(go.Scatter(
        x=x,
        y=refs["IQ ADX"].astype(float).tolist(),
        mode="lines+markers",
        name="IQ Option",
        line=dict(width=4),
        marker=dict(size=8),
    ...
    for method_label in ranking.head(top_n)["Método"].tolist():
        if method_label == "DX sin suavizar":
            series = dx
        elif method_label.startswith("RMA primer valor"):
            period = int(method_label.replace("RMA primer valor", ""))
            series = _apply_final_smoother(dx, "RMA_FIRST", period)
        elif method_label.startswith("RMA"):
            period = int(method_label.replace("RMA", ""))
            series = _apply_final_smoother(dx, "RMA", period)
        elif method_label.startswith("SMA"):
            period = int(method_label.replace("SMA", ""))
            series = _apply_final_smoother(dx, "SMA", period)
        elif method_label.startswith("EMA"):
            period = int(method_label.replace("EMA", ""))
            series = _apply_final_smoother(dx, "EMA", period)
        else:
            continue

        y = [series.loc[idx] for idx in ref_indices]
        fig.add_trace(go.Scatter(
            x=x,
            y=y,
            mode="lines+markers",
    ...
    fig.update_yaxes(range=[0, 100], title_text="ADX")
    fig.update_layout(
        title="IQ Option vs mejores candidatos DX→ADX",
        height=560,
        margin=dict(l=30, r=30, t=60, b=30),
        hovermode="x unified",
        legend=dict(
