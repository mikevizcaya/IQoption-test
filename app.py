...
from pdf_report import create_event_pdf
from charts import (
    build_chart,
    build_recent_chart,
    build_adx_diagnostic_chart,
    build_adx_diagnostic_table,
    build_adx_components_table,
    build_rma_comparison_table,
    build_rma_comparison_chart,
    build_adx_final_stage_table,
    build_adx_final_stage_chart,
    rank_adx_final_smoothers,
    build_best_smoothers_chart,
)
...
if st.session_state.screen == "download":
    ...
    # Usuario prellenado por default para no tener que escribirlo cada vez.
    # Si existe IQ_EMAIL en Streamlit Secrets, ese valor tendrá prioridad.
    default_email = "mike_vizcaya@yahoo.com"
    try:
        default_email = st.secrets.get("IQ_EMAIL", default_email)
    except Exception:
        pass
    ...
    if events.empty:
        ...
        if st.session_state.selected_event_source_index is None:
            ...
            pdf_left, pdf_right = st.columns([3, 1])
            with pdf_left:
                st.caption(
                    "PDF carta vertical de 2 páginas: página 1 = Estándar + Antes; "
                    "página 2 = Inicio + Fin. Usa el fondo seleccionado."
                )
            ...
            with pdf_right:
                if st.button(
                    ...
                    with st.spinner("Creando PDF..."):
                        try:
                            st.session_state.pdf_bytes = create_event_pdf(
                                analyzed=analyzed,
                                source_index=st.session_state.selected_event_source_index,
                                asset_label=st.session_state.selected_asset,
                            ...
                            st.session_state.pdf_bytes = None
                            st.error(f"No fue posible crear el PDF: {exc}")

                if st.session_state.get("pdf_bytes"):
                    selected_pdf_event = analyzed.loc[
                        st.session_state.selected_event_source_index
                    ]
                    safe_time = str(
                        selected_pdf_event["datetime_mexico"]
                    ).replace(":", "-").replace(" ", "_")
                    st.download_button(
                        "Descargar PDF",
                        data=st.session_state.pdf_bytes,
                        file_name=f"Trading_Analitic_{safe_time}.pdf",
            ...
            if show_adx_diag:
                ...
                st.subheader("Desglose interno del ADX")
                st.caption(
                    "Esta tabla muestra cada etapa del cálculo. "
                    "Sirve para localizar exactamente dónde empieza la diferencia "
                    "contra IQ Option."
                )

                components_table = build_adx_components_table(
                    analyzed=analyzed,
                    source_index=st.session_state.selected_event_source_index,
                    radius=4,
                    adx_period=7,
                ...
                st.dataframe(
                    ...
                    hide_index=True,
                    column_config={
                        "High": st.column_config.NumberColumn(format="%.6f"),
                        "Low": st.column_config.NumberColumn(format="%.6f"),
                        "Close": st.column_config.NumberColumn(format="%.6f"),
                        "TR": st.column_config.NumberColumn(format="%.6f"),
                        "ATR": st.column_config.NumberColumn(format="%.6f"),
                        "+DM": st.column_config.NumberColumn(format="%.6f"),
                        "-DM": st.column_config.NumberColumn(format="%.6f"),
                        "+DM/ATR": st.column_config.NumberColumn(format="%.6f"),
                        "-DM/ATR": st.column_config.NumberColumn(format="%.6f"),
                        "+DI": st.column_config.NumberColumn(format="%.4f"),
                        "-DI": st.column_config.NumberColumn(format="%.4f"),
                        "DX": st.column_config.NumberColumn(format="%.4f"),
                        "ADX": st.column_config.NumberColumn(format="%.4f"),
                    },
                )

                st.markdown(
                    "**Ruta del cálculo:** "
                    "`TR → ATR → +DM/-DM → DM/ATR → +DI/-DI → DX → ADX`"
                )

                st.subheader("Prueba de variantes RMA")
                st.caption(
                    "Aquí mantenemos fija toda la fórmula del ADX y cambiamos "
                    "únicamente cómo se inicializa/suaviza la RMA. "
                    "Compara estas curvas con IQ Option."
                )

                rma_fig = build_rma_comparison_chart(
                    analyzed=analyzed,
                    source_index=st.session_state.selected_event_source_index,
                    radius=8,
                    theme=chart_theme,
                ...
                rma_table = build_rma_comparison_table(
                    analyzed=analyzed,
                    source_index=st.session_state.selected_event_source_index,
                    radius=6,
                )
                st.dataframe(
                    ...
                    hide_index=True,
                    column_config={
                        "RMA semilla SMA": st.column_config.NumberColumn(format="%.4f"),
                        "RMA primer valor": st.column_config.NumberColumn(format="%.4f"),
                        "SMA móvil": st.column_config.NumberColumn(format="%.4f"),
                        "RMA semilla 0": st.column_config.NumberColumn(format="%.4f"),
                    },
                )

                st.info(
                    "Si una de estas variantes reproduce de forma consistente "
                    "los niveles de IQ Option (por ejemplo 71–79 en las velas "
                    "que estás comparando), habremos identificado el tipo de "
                    "inicialización/suavizado que usa."
                )

                st.subheader("Prueba final: DX → ADX")
                st.caption(
                    "Aquí dejamos fijo todo el cálculo hasta DX y cambiamos "
                    "únicamente la última transformación. Así podemos comprobar "
                    "si IQ Option muestra DX casi directo o usa RMA7, SMA7, EMA7 "
                    "u otra inicialización de RMA."
                )

                final_stage_fig = build_adx_final_stage_chart(
                    analyzed=analyzed,
                    source_index=st.session_state.selected_event_source_index,
                    radius=8,
                    theme=chart_theme,
                ...
                final_stage_table = build_adx_final_stage_table(
                    analyzed=analyzed,
                    source_index=st.session_state.selected_event_source_index,
                    radius=6,
                    period=7,
                ...
                st.dataframe(
                    ...
                    hide_index=True,
                    column_config={
                        "DX sin suavizar": st.column_config.NumberColumn(format="%.4f"),
                        "ADX RMA7": st.column_config.NumberColumn(format="%.4f"),
                        "ADX SMA7": st.column_config.NumberColumn(format="%.4f"),
                        "ADX EMA7": st.column_config.NumberColumn(format="%.4f"),
                        "ADX RMA7 primer valor": st.column_config.NumberColumn(format="%.4f"),
                    },
                )

                st.info(
                    "La comparación decisiva es entre estos valores y la línea "
                    "amarilla de IQ Option en las mismas horas. Si DX coincide "
                    "mejor que todas las versiones suavizadas, IQ Option estaría "
                    "aplicando un suavizado final mucho menor o diferente."
                )

                st.subheader("Buscador automático de coincidencia con IQ Option")
                st.caption(
                    "Introduce algunos valores reales del ADX amarillo de IQ Option. "
                    "El analizador probará automáticamente DX, RMA, SMA y EMA con "
                    "períodos de 2 a 14 y los ordenará por error medio absoluto."
                )

                selected_idx = st.session_state.selected_event_source_index
                auto_start = max(0, selected_idx - 4)
                auto_end = min(len(analyzed) - 1, selected_idx + 4)

                default_ref = pd.DataFrame({
                    "Hora": pd.to_datetime(
                        analyzed.loc[auto_start:auto_end, "datetime_mexico"]
                    ).dt.strftime("%H:%M:%S").values,
                    "IQ ADX": [None] * (auto_end - auto_start + 1),
                })

                # Precargamos los valores que ya mediste en IQ Option si esas horas
                # están presentes. Se pueden editar libremente.
                known_iq = {
                    "19:56:00": 71.2296,
                    "19:57:00": 71.7647,
                    "19:58:00": 75.5102,
                ...
                default_ref["IQ ADX"] = [
                    known_iq.get(h, None) for h in default_ref["Hora"]
                ]

                iq_ref = st.data_editor(
                    default_ref,
                    use_container_width=True,
                    hide_index=True,
                    num_rows="fixed",
                    key="iq_adx_reference_editor",
                    column_config={
                        "Hora": st.column_config.TextColumn("Hora"),
                        "IQ ADX": st.column_config.NumberColumn(
                            "IQ ADX",
                            min_value=0.0,
                            max_value=100.0,
                            format="%.4f",
                ...
                valid_ref_count = pd.to_numeric(
                    iq_ref["IQ ADX"],
                    errors="coerce",
                ).notna().sum()

                if valid_ref_count >= 2:
                    ranking = rank_adx_final_smoothers(
                        analyzed=analyzed,
                        iq_reference=iq_ref,
                        min_period=2,
                        max_period=14,
                    ...
                    if not ranking.empty:
                        ...
                        st.dataframe(
                            ...
                            column_config={
                                ...
                                "Sesgo": st.column_config.NumberColumn(format="%.4f"),
                                "Error máximo": st.column_config.NumberColumn(format="%.4f"),
                            },
                        )

                        best_fig = build_best_smoothers_chart(
                            analyzed=analyzed,
                            iq_reference=iq_ref,
                            ranking=ranking,
                            theme=chart_theme,
                        ...
                        best = ranking.iloc[0]
                        st.success(
                            f"Mejor candidato actual: {best['Método']} "
                            f"(MAE={best['MAE']:.4f}, RMSE={best['RMSE']:.4f})."
                        )
                else:
                    st.info(
                        "Introduce al menos dos valores reales de IQ Option "
                        "para ejecutar el ranking automático."
                    )
